package src;
/**
 * Purpose: A CPU src.Blackjack player using the Basic Strat Chart
 * Author: Alex Li
...
public class BasicStratPlayer extends Player {
	...
	public BasicStratPlayer(double bankroll) {
		super(bankroll);
	}
	...
	public ArrayList<PlayerHand> play(Card dealerCard) {
		for (int i = 0; i < hands().size(); i++) {
			...
			while (!hands().get(i).isDone()) {
				applyMove(hands().get(i), dealerCard, getNextMove(hands().get(i), dealerCard));
			}
		}
		return hands();
	}
	...
	@Override
	// src.Player is ruined when having less one betting unit
