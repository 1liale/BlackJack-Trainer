package src; /**
 * Purpose: A CPU src.Blackjack player with a high-low strategy and the illustrious 18 deviations
...
public class CardCountingPlayer extends Player{
	...
	public CardCountingPlayer(double bankroll) {
		super(bankroll);
	...
	protected static String applyChart(PlayerHand playerHand, Card dealerCard, int chartNum) {
		//Handle deviations
		double trueCount = Blackjack.shoe().trueCount();
		int playerVal = playerHand.val(), dealerVal = dealerCard.val(), decks = Blackjack.shoe().decks();
		switch (chartNum) {
		case 0:
			// Rank 2
			...
			if (playerVal == 13 && dealerVal == 3 && (trueCount <= -2.0 || (decks == 1 && trueCount <= -1.0))) {
				System.out.println("Rank 18");
				return "H";
			}
			break;
		case 2:
			// Rank 4, 5
			if (playerVal == 20 && ((dealerVal == 5 && trueCount >= 5.0)
					|| (dealerVal == 6 && trueCount >= 4.0))) {
				return "SP";
			}
		}
	...
	public ArrayList<PlayerHand> placeInsurances() {
		// Rank 1
		if (Blackjack.shoe().trueCount() >= 3.0 || (Blackjack.shoe().decks() >= 2 && Blackjack.shoe().trueCount() >= 2.4)
				|| (Blackjack.shoe().decks() == 1 && Blackjack.shoe().trueCount() >= 1.4))
	...
	public ArrayList<PlayerHand> play(Card dealerCard) {
		for (int i = 0; i < hands().size(); i++) {
			...
			while (!hands().get(i).isDone()) {
				String basicMove = BasicStratPlayer.getNextMove(hands().get(i), dealerCard);
				String cardCountingMove = getNextMove(hands().get(i), dealerCard);
				applyMove(hands().get(i), dealerCard, cardCountingMove );
				if (!basicMove.equals(cardCountingMove)) {
