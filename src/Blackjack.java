package src; /**
 * Console Based src.Blackjack Game Trainer
 * Standard src.Blackjack Rules: H17, DAS, RAS, 3:2 payout
 * Require UTF8 encoding to view properly (see src.Card for more info)
 * Author: Yifan Zong
...
public class Blackjack {
	...
	public static void choosePlayer() {
		...
		for (int i = 0; i < players.length; i++) {
			...
			System.out.println("3. src.Card Counter AI");
			while (true) {
				try {
	...
	public static void main(String[] args) {
		...
		while (true) {
			while (shoe.length() > cutLength) {
				...
				for (int i = 0; i < players.length; i++) {
					if (!players[i].ruined()) {
						System.out.println("src.Player" + i + ":");
				...
				System.out.println("src.Dealer: [" + dealerCard +"]");
				for (int i = 0; i < players.length; i++) {
					if (!players[i].ruined()) {
						players[i].newHand();
						System.out.println("src.Player" + i + ": " + players[i].hands().get(0));
					}
				}
				System.out.println();
				...
				if (dealerCard.val() == 11) {
					...
					if (dealer.hand().val() == 21) {
						System.out.println("src.Dealer: " + dealer.reveal()); //reveal() updates the running count
						for (int i = 0; i < players.length; i++) {
							if (! players[i].ruined()) {
								System.out.println("src.Player" + i + ":");
								for (int j = 0; j <  players[i].hands().size(); j++) {
									...
									if (playerHands.get(j).val() == 21) { //If player also has blackjack
										System.out.println(" src.Blackjack!");
				...
				for (int i = 0; i < players.length; i++) {
					if (!players[i].ruined()) {
						System.out.println("src.Player" + i + ": ");
						players[i].play(dealerCard); //src.Dealer's card is passed as parameter
				...
				//src.Dealer does not play if all players have busted
				if (allBusted) {
					System.out.println("All players have busted.");
					System.out.println("src.Dealer: " + dealer.reveal());
					System.out.println();
					continue;
				}
				
				//src.Dealer plays their hand
				boolean dealerBlackjack = false, dealerBusted = false;
				System.out.print("src.Dealer: " + dealer.play());
				...
				for (int i = 0; i < players.length; i++) {
					...
					if (!player.ruined()) {
						...
						for (int j = 0; j < playerHands.size(); j++) {
							...
							//src.Player has blackjack but dealer does not
							...
							//src.Dealer busts but player does not or player has a higher non-blackjack hand
							if (dealerBusted || playerHands.get(j).val() > dealer.hand().val()) {
								System.out.println(" Win");
								player.update(2.0 * playerHands.get(j).bet());
								continue;
							}
							//src.Player does not bust but has a lower value hand
							if (playerHands.get(j).val() <  dealer.hand().val()) {
								System.out.println(" Lose");
								player.update(0.0);
								continue;
